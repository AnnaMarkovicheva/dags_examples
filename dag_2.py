...
def max_EU_Sales():
    ...
    """
    df_my_year = pd.read_csv(SALES_YEAR)
    max_EU_sales = df_my_year.groupby('Genre', as_index=False) \
        .agg({'EU_Sales': 'sum'}) \
        .sort_values('EU_Sales', ascending=False).EU_Sales.iloc[0]
    df_EU_Sales = df_my_year.groupby('Genre', as_index=False) \
        .agg({'EU_Sales': 'sum'}) \
...
def max_platf_NA():
    ...
    """
    df_my_year = pd.read_csv(SALES_YEAR)
    df_platf_NA = df_my_year[df_my_year.NA_Sales > 1] \
        .groupby('Platform', as_index=False) \
        .agg({'Name': 'count'}) \
        .sort_values('Name', ascending=False) \
        .rename(columns={'Name': 'Number_of_Games'})
    max_count = df_my_year[df_my_year.NA_Sales > 1] \
        .groupby('Platform', as_index=False) \
        .agg({'Name': 'count'}) \
        .sort_values('Name', ascending=False) \
        .rename(columns={'Name': 'Number_of_Games'}).Number_of_Games.iloc[0]
...
def max_avg_sales_JP():
    ...
    """
    df_my_year = pd.read_csv(SALES_YEAR)
    df_avg_sales_JP = df_my_year.groupby('Publisher', as_index=False) \
        .agg({'JP_Sales': 'mean'}) \
        .sort_values('JP_Sales', ascending=False) \
        .rename(columns={'JP_Sales': 'JP_Avg_Sales'})
    max_sales = df_my_year.groupby('Publisher', as_index=False) \
        .agg({'JP_Sales': 'mean'}) \
        .sort_values('JP_Sales', ascending=False) \
        .rename(columns={'JP_Sales': 'JP_Avg_Sales'}).JP_Avg_Sales.iloc[0]
